 ...
 * @route       PUT /api/v1/auth/updatepassowrd
 * @access      Private
 */
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new ErrorResponse(`There is no user with email ${req.body.email}`, 401)
        );
    }
    ...
    try {
        ...
        res.status(202).json({
            success: true,
            data: 'If this email is right you\'ll receive further instructions in your inbox'
        });
    } catch (err) {
        console.log(err);
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new ErrorResponse(`Invalid reset password token`, 401)
        );
    }
